...
class download():
    ...
    def __init__(self):
        ...
        pattern=re.compile('<tr class=.*?>.*?<td>(.*?)</td>.*?<td>(.*?)</td>.*?</tr>',re.S)
        results=re.findall(pattern,html.text)
        for result in results:
            a,b=result
            ip=a+':'+b  
            self.iplist.append(ip.strip)##添加到我们上面初始化的list里面
    ...
    def get(self, url, timeout,referer=None, proxy=None, num_retries=6): ##给函数一个默认参数proxy为空
        ...
        if proxy == None: ##当代理为空时，不使用代理获取response（别忘了response啥哦！之前说过了！！）
            try:
                ...
                if num_retries > 0: ##num_retries是我们限定的重试次数
                    ...
                    return self.get(url, timeout,self.referer,num_retries-1)  ##调用自身 并将次数减1
                else:
                    print(u'开始使用代理')
                    time.sleep(10)
                    IP = ''.join(str(random.choice(self.iplist)).strip()) ##下面有解释哦
                    proxy = {'http': IP}
                    return self.get(url, timeout,self.referer, proxy,) ##代理不为空的时候
            ...
            try:
                ...
                if num_retries > 0:
                    ...
                    print(u'当前代理是：', proxy)
                    return self.get(url, timeout,self.referer, proxy, num_retries - 1)
                else:
                    print(u'代理也不好使了！取消代理')
                    return self.get(url,self.referer, 3)
