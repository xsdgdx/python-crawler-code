...
class download():
    ...
    def __init__(self):
        ...
        for result in results:
            a,b=result
            ip=a+':'+b
            self.iplist.append(ip)##添加到我们上面初始化的list里面
        print(self.iplist)
        ...
        if proxy == None: ##当代理为空时，不使用代理获取response（别忘了response啥哦！之前说过了！！）
            try:
                ...
                if num_retries > 0: ##num_retries是我们限定的重试次数
                    ...
                    return self.get(url, timeout,referer,num_retries-1)  ##调用自身 并将次数减1
                else:
                    print(u'开始使用代理')
                    time.sleep(10)
                    IP = ''.join(str(random.choice(self.iplist)).strip()) ##下面有解释哦
                    proxy = {'http': IP}
                    return self.get(url, timeout,referer, proxy,) ##代理不为空的时候
            ...
            try:
                ...
                if num_retries > 0:
                    ...
                    print(u'当前代理是：', proxy)
                    return self.get(url, timeout,referer, proxy, num_retries - 1)
                else:
                    print(u'代理也不好使了！取消代理')
                    return self.get(url,referer, 3)
